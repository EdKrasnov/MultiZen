...
### LiteSpeed Cache
Кэширует ответы фидов. `/feed/` добавлен в исключения кэша LiteSpeed.
При отладке использовать `?nocache=1` или `wp litespeed-purge all`.
...
## 10. Changelog (сессия 2026-09-23/24)

1. **Разобрали конфликт** старого v1 (`rss-for-yandex-zen`) и форка
   (`multi-rss-for-zen`). Причина падения — совпадающие имена функций.
2. **Замена префиксов** `yzen_` → `mzen_` через `sed`.
3. **git-репозиторий** на сервере, ветка `v2-multi-channels`, решение
   проблемы `safe.directory`.
4. **Починили структуру ветки** — файлы плагина перенесены из вложенной папки
   в корень репозитория (`git rm -r multi-rss-for-zen` → `git add -A` → rename).
5. **Полный rewrite v2**:
   - `MZEN_Plugin` — синглтон, boot
   - `MZEN_Channels` — CRUD, поиск, дублирование, JSON
   - `MZEN_Migrator` — миграция v1 → v2
   - `MZEN_Helpers` — утилиты
   - `MZEN_Feed` — фиды
   - `MZEN_Metabox` — метабокс + JS
   - `MZEN_Admin_Settings` — страница настроек
   - `MZEN_Admin_Columns` — колонка + фильтр + bulk actions
   - `templates/feed-rss2.php` — XML-шаблон
6. **Успешная активация**. Канал `multizen` создан из опций v1.
7. **Решение**: НЕ назначать все записи на канал по умолчанию
   (убрали `assign_orphan_posts` — 300к лишних строк).
8. **Отказ от донат-блока**, переименование в `Мульти.Дзен`.
9. **Логика `excludedefault`**:
   - PHP сохраняет фактическое состояние чекбокса
   - JS ставит галочку при выборе канала с `excludedefault = enabled`
10. **Bulk assign** очищает канало-зависимые мета, чтобы записи следовали настройкам канала.
11. **Чистка legacy `_mzen_rss_enabled = no`** (6 записей от v1).
12. **Реализована отложенная публикация `_mzen_publish_at`**:
    - UI: `datetime-local` + кнопка «Очистить» + подсказка минимума
    - JS: валидация (не раньше `post_date + delay_minutes`)
    - PHP: сохранение с серверной валидацией
    - Feed: условие в `meta_query`
